...
- **Stripboard (Veroboard) support** — set `grid.type: "strip"` (+ `stripAxis: "row"|"col"`) and the board models copper strips: holes on one strip are electrically connected unless a **track cut** (`trackCuts`) breaks them. The signature stripboard ERC — two different nets sharing one uncut strip segment (`stripShorts`) — is a fab-blocking error in both engines; the editor draws the strips, a **cut mode** (key 6) toggles track cuts between adjacent holes, and the command palette switches board type / strip axis.
