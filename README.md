...
- **Config scaffolder** — `solver.py --emit-config` derives a best-effort, review-me `perfwire_config.json` from a board (net classes by name/role, rail_rank, power_entry, decoupling seeded from nearest caps, single-lead allowlist) so the audit isn't dark on a fresh board.
