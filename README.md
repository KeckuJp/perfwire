...
- **Guard-ring synthesis** — `solver.py --guard <net>` and the editor's guard command turn the high-Z guard advisory into an applied guard ring: the surrounding free holes become leads of the inferred buffer-output net, offered as a new proposal (never auto-applied).
- **Value-aware EE** (optional inputs) — with a per-part `value` (Ω/F) and a `rail_volts` net→volts map: resistor power-dissipation check, decoupling-value sanity, and an output-to-power pin-conflict check.
...
- A routed crosstalk model (today: an endpoint-segment heuristic, since jumper routes aren't modelled). Guard-ring synthesis, star/daisy ground-topology scoring, high-Z guard advisories, keep-away re-audit, power-reachability ERC, config scaffolding, build-packet export, fix suggestions, multi-candidate placement search and value-aware EE checks already ship.
