...
- **Fix suggestions + placement search** — every ERC/EE finding carries a machine-actionable remedy (`ee.fixes`, shown in the audit panel); `solver.py --propose-n` and the editor's auto-search try a weight grid and keep the best by (errors, wire count).
