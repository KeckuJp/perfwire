...
**日本のユーザーへ**: `mesh`（十字配線基板）に対応している——秋月電子のP-09694やサンハヤトの
UB-WRD01のような、全穴が上下左右の隣接穴と繋がった状態で出荷される基板。未カットのまま設計を
乗せると全ネットが共通ラインで短絡する（電源とGNDが格子経由で繋がる、という定番の「電源が入らない」
原因）。`trackCuts`で銅箔ブリッジを切って独立ネットを作り、エディタはカットモード（キー6）と
必要カット箇所の自動生成（各ネットを領域成長させ境界を切る）を提供する。

その他の特長（詳細は[英語版](README.md#features)を参照——技術的な内容は言語間で同一）:
