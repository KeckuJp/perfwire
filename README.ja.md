...
**あなたのための、エディタ**
- **3Dビュー** — ドラッグで回転、ホイールでズーム。部品は曲がったリードとはんだフィレット付きの実形状で描画。上部のツールバーでレイヤー表示切替・ネット分離表示・X線表示・ホバーで接続先ツールチップ・密集配線の判読性整形ができ、凡例のネット一覧からクリックでネット固定表示や部品の接続一覧パネルも呼び出せる。
